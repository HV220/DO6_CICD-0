...
void scan_files(int argc, char **argv, t_pattern *patterns, int patterns_cnt,
                int *flags) {
...
void files_processing(t_pattern *patterns, int patterns_cnt, t_file *files,
                      int files_cnt, int *flags) {
...
int seek_in_file(FILE *fd, t_pattern *patterns, int patterns_cnt, int *flags,
...
void get_files(int argc, char **argv, t_pattern *patterns, t_file *files,
               int *flags) {
    int same = 0;
    int k = 0;
...
int is_arg_a_file(int i, char **argv, t_pattern *patterns, int *same,
                  const int *flags) {
    return (!strspn(argv[i], "-") &&
            !((strchr(argv[i], 'f') && strspn(argv[i], "-")) ||
              ...
              (i != 1 && argv[i - 1][strlen(argv[i - 1]) - 1] == 'e' &&
               strspn(argv[i - 1], "-")) ||
              (!(flags[8] || flags[0]) && !strcmp(argv[i], patterns[0].name) &&
               ((*same)--) >= 0)));
}
...
void get_grep_flags(int argc, char **argv, int *flags, t_pattern *patterns,
                    int *patterns_cnt) {
    int e = 0, f = 0;
    for (int i = 1; i < argc; i++) {
        if (strspn(argv[i], "-")) {
